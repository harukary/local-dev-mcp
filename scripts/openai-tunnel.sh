...
case "${1:-}" in
  ...
  -h|--help)
    cat <<'USAGE'
usage: openai-tunnel.sh [--run|--doctor]
...
Required:
  LOCAL_DEV_MCP_AUTH_MODE=openai-tunnel
...
if [ "$#" -ne 0 ]; then
  echo "usage: openai-tunnel.sh [--run|--doctor]" >&2
  exit 64
fi
...
if [ -n "${LOCAL_DEV_MCP_AUTH_MODE:-}" ] && [ "$LOCAL_DEV_MCP_AUTH_MODE" != "openai-tunnel" ]; then
  echo "[openai-tunnel] LOCAL_DEV_MCP_AUTH_MODE must be openai-tunnel when explicitly set." >&2
  exit 1
fi
