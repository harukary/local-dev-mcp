...
usage() {
  cat <<'USAGE'
usage: install-openai-tunnel-launchd.sh [--install-only|--activate]
...
write_agent "$SERVER_LABEL" "$SERVER_LABEL_XML" "openai-tunnel-server.sh" "openai-mcp-server.log"
write_agent "$TUNNEL_LABEL" "$TUNNEL_LABEL_XML" "openai-tunnel.sh" "openai-tunnel.log"
