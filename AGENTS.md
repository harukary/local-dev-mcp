...
- HTTP MCPはloopback bind + `X-Local-Dev-MCP-Tunnel-Token`を安全境界とし、tool/resource accessは`openai/subject` allowlistでもfail-closedに制限する。stdio transportはlocal MCP client向けに独立して維持する。
