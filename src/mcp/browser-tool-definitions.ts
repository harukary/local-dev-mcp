...
export function buildBrowserToolDefinitions() {
  return [
    ...
    { name: "browser.stop", description: "Stop the current chat-owned browser and checkpoint it. The live browser/profile critical section is released before slower snapshot promotion maintenance, so later browser operations are not blocked by that maintenance. Repeated stop is idempotent once the browser is already idle.", inputSchema: { type: "object", properties: {} }, annotations: WA },
